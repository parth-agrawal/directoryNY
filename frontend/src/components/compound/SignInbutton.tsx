...
const SignInButton = () => {
    ...
    const handleSignIn = async () => {
        try {
            ...
            // await request.json();
            if (request.ok) {
                navigate('/'); // Ensure this is reached
            }
        } catch (error) {
    ...
    return (
        <>
            {!user &&
                <button onClick={handleSignIn} type="button" className="text-black hover:bg-[#1da1f2]/50 focus:ring-4 focus:outline-none focus:ring-[#1da1f2]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 mb-2">
